    ...
    const handlePurchase = async (packId: 'starter' | 'pro') => {
        ...
        try {
            ...
            const { data, error } = await supabase.functions.invoke('create-checkout', {
                body: { packId, type }, // Pass type to backend
            });
                ...
                <div className="p-8">
                    <div className="text-center mb-10">
                        <h2 className="text-2xl font-bold text-slate-900">
                            {isApi ? 'Top Up API Credits' : 'Top Up Your Credits'}
                        </h2>
                        <p className="text-slate-500 mt-2">
                            {isApi
                                ? 'Scale your application with reliable generation credits.'
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="border border-slate-200 rounded-xl p-6 hover:border-brand-200 hover:shadow-lg transition-all relative group">
                            ...
                            <div className="text-center mb-6 mt-2">
                                <div className="text-4xl font-bold text-slate-900 mb-1">{isApi ? '$6.49' : '$7.49'}</div>
                            ...
                            <div className="flex items-center justify-center gap-2 mb-6 text-brand-600 font-bold text-lg">
                                ...
                                {isApi ? '50 API Calls' : '50 Credits'}
                            </div>

                            <ul className="space-y-3 mb-8 text-sm text-slate-600">
                                <li className="flex items-center gap-2"><Check size={16} className="text-green-500" /> {isApi ? '50 Generations' : '50 High-Res Generations'}</li>
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xl relative overflow-hidden">
                            ...
                            <div className="text-center mb-6 mt-2">
                                <div className="text-4xl font-bold text-slate-900 mb-1">{isApi ? '$20.00' : '$25.00'}</div>
                                <div className="text-slate-500 text-sm">One-time payment</div>
                            </div>
                            ...
                            <div className="flex items-center justify-center gap-2 mb-6 text-red-600 font-bold text-lg">
                                ...
                                {isApi ? '200 API Calls' : '200 Credits'}
                            </div>

                            <ul className="space-y-3 mb-8 text-sm text-slate-600">
                                <li className="flex items-center gap-2"><Check size={16} className="text-green-500" /> {isApi ? '200 Generations' : '200 High-Res Generations'}</li>
                                <li className="flex items-center gap-2"><Check size={16} className="text-green-500" /> Commercial Usage Rights</li>
