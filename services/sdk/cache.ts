...
export class WebCacheManager implements CacheManager {
  ...
  private openDB(): Promise<IDBDatabase> {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error('[WebCacheManager] Failed to open database:', request.error);
  ...
  async get(key: string): Promise<string | null> {
    ...
    try {
      ...
      console.warn('[WebCacheManager] Cache retrieval failed:', error);
      return null;
    }
  }
  ...
  async set(key: string, data: Blob | string): Promise<string> {
    ...
    try {
      ...
      console.warn('[WebCacheManager] Cache storage failed:', error);
      if (typeof data === 'string') return data;
      return URL.createObjectURL(data);
    }
  }
  ...
  async delete(key: string): Promise<void> {
    ...
    try {
      ...
      console.warn('[WebCacheManager] Cache deletion failed:', error);
    }
  }
  ...
  async clear(): Promise<void> {
    ...
    try {
      ...
      console.warn('[WebCacheManager] Cache clear failed:', error);
    }
  }
  ...
  private async enforceLimit(db: IDBDatabase): Promise<void> {
    try {
      ...
      console.warn('[WebCacheManager] Failed to enforce cache limit:', error);
    }
  }
  ...
  private async cleanup(): Promise<void> {
    ...
    try {
      ...
      if (expiredKeys.length > 0) {
        console.log(`[WebCacheManager] Cleaned up ${expiredKeys.length} expired entries`);
      }
    } catch (error) {
      console.warn('[WebCacheManager] Cleanup failed:', error);
    }
  }
}
